...
export default function CourseViewer({
  ...
  // Generate course outline on mount — static cache → Firebase → AI → fallback
  useEffect(() => {
    let cancelled = false;
    const topicSlug = slugify(topic);
    ...
    async function loadOutline() {
      // 1. Try AIService (static bundled JSON cache — zero latency, covers 27 topics)
      //    This also checks in-memory + Firebase shared caches before going to AI.
      //    We use a short race so cached responses come back instantly.
      try {
        const aiResult = await Promise.race([
          AIService.call('courseOutline', { topic, domain, ageGroup, personality, topInterests }),
          new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 8000)),
        ]);
        if (cancelled) return;
        const parsed = parseOutline(aiResult);
        if (parsed?.length) {
          applyOutlineMeta(aiResult, parsed);
          setPhase('outline');
          // Back-fill Firebase cache for other users
          if (aiResult && typeof aiResult === 'object') {
            saveCourseOutline(topicSlug, aiResult).catch(() => {});
          }
          return;
        }
      } catch { /* fall through to Firebase then fallback */ }

      if (cancelled) return;

      // 2. Firebase pre-warmed cache (community-shared outlines saved by previous users)
      try {
        const firebaseCourse = await Promise.race([
          loadCourseOutline(topicSlug),
      ...
      // 3. Immediate fallback — never leave the user stuck on the generating screen
      setOutline(buildFallbackOutline(topic, domain));
      setPhase('outline');

      // 4. Try enriching in background with Wikipedia + AI (update outline if successful)
